...
class GesturePainter(HandDetector):
    ...
    LINE_THICKNESS = (
        10  # 잘못된 선이 그려지는 것을 방지하기 위한 최대 허용 거리
    )
    ...
    def is_drawing_mode(self, lm_list):
        ...
        is_index_up = (
            lm_list[self.INDEX_FINGER_TIP][
                y_idx
            ]  # 검지 끝이 검지 중간관절보다 위에 있으면
            < lm_list[self.INDEX_FINGER_PIP][y_idx]  # 검지가 펴진 상태로 판단
        )
        is_middle_down = (
            lm_list[self.MIDDLE_FINGER_TIP][
                y_idx
            ]  # 중지 끝이 중간관절보다 아래에 있으면
    ...
    def draw_canvas(self, frame, lm_list):
        ...
        foreground = cv2.bitwise_and(
            self.canvas, self.canvas, mask=mask
        )  # 캔버스에서 그림이 있는 부분만 추출
        background = cv2.bitwise_and(
            frame, frame, mask=cv2.bitwise_not(mask)
        )  # 원본 영상에서 그림이 없는 부분만 추출
